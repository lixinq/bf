...
class company extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Reviews.Company.View');
	...
	*/
	...
	{
		$this->auth->restrict('Reviews.Company.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_reviews())
			{
				...
				Template::redirect(SITE_AREA .'/company/reviews');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/company/reviews');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Reviews.Company.Edit');
		...
		{
			$this->auth->restrict('Reviews.Company.Delete');
