...
class settings extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Reviews.Settings.View');
	...
	*/
	...
	{
		$this->auth->restrict('Reviews.Settings.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_reviews())
			{
				...
				Template::redirect(SITE_AREA .'/settings/reviews');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/settings/reviews');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Reviews.Settings.Edit');
		...
		{
			$this->auth->restrict('Reviews.Settings.Delete');
