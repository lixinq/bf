...
class settings extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Video.Settings.View');
	...
	*/
	...
	{
		$this->auth->restrict('Video.Settings.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_video())
			{
				...
				Template::redirect(SITE_AREA .'/settings/video');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/settings/video');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Video.Settings.Edit');
		...
		{
			$this->auth->restrict('Video.Settings.Delete');
