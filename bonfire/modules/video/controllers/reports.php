...
class reports extends Admin_Controller {
	...
	*/
	...
	{
		$this->auth->restrict('Video.Reports.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_video())
			{
				...
				Template::redirect(SITE_AREA .'/reports/video');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/reports/video');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Video.Reports.Edit');
		...
		{
			$this->auth->restrict('Video.Reports.Delete');
