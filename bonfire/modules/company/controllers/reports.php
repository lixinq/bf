...
class reports extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Company.Reports.View');
	...
	*/
	...
	{
		$this->auth->restrict('Company.Reports.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_company())
			{
				...
				Template::redirect(SITE_AREA .'/reports/company');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/reports/company');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Company.Reports.Edit');
		...
		{
			$this->auth->restrict('Company.Reports.Delete');
