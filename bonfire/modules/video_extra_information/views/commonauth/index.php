<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Video_extra_information.Commonauth.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Video_extra_information.Commonauth.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/commonauth/video_extra_information/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->video_extra_information_video_id) ?></td>
				<?php else: ?>
