<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('User_Info.Commonauth.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('User_Info.Commonauth.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/commonauth/user_info/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->user_info_user_id) ?></td>
				<?php else: ?>
