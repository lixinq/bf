...
class content extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('User_Info.Content.View');
	...
	*/
	...
	{
		$this->auth->restrict('User_Info.Content.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_user_info())
			{
				...
				Template::redirect(SITE_AREA .'/content/user_info');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/content/user_info');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('User_Info.Content.Edit');
		...
		{
			$this->auth->restrict('User_Info.Content.Delete');
