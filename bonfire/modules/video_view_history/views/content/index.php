<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Video_view_history.Content.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Video_view_history.Content.Delete')) : ?>
				<tr>
					<td colspan="5">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Video_view_history.Content.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/content/video_view_history/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->video_view_history_video_id) ?></td>
				<?php else: ?>
