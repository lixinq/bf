...
class view extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Video_View_History.View.View');
		$this->load->model('video_view_history_model', null, true);
		$this->lang->load('video_view_history');
		
		Template::set_block('sub_nav', 'view/_sub_nav');
	}
	...
	*/
	...
	{
		$this->auth->restrict('Video_View_History.View.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_video_view_history())
			{
				...
				Template::redirect(SITE_AREA .'/view/video_view_history');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/view/video_view_history');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Video_View_History.View.Edit');
		...
		{
			$this->auth->restrict('Video_View_History.View.Delete');
	...
	*/
	private function save_video_view_history($type='insert', $id=0)
	{
		...
		$this->form_validation->set_rules('video_view_history_video_id','Video ID','max_length[11]');
		$this->form_validation->set_rules('video_view_history_user_id','User ID','max_length[20]');
