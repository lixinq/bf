...
class reports extends Admin_Controller {
	...
	*/
	...
	{
		$this->auth->restrict('Video_view_history.Reports.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_video_view_history())
			{
				...
				Template::redirect(SITE_AREA .'/reports/video_view_history');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/reports/video_view_history');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Video_view_history.Reports.Edit');
		...
		{
			$this->auth->restrict('Video_view_history.Reports.Delete');
