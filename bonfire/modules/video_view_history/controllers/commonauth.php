...
class commonauth extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Video_view_history.Commonauth.View');
	...
	*/
	...
	{
		$this->auth->restrict('Video_view_history.Commonauth.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_video_view_history())
			{
				...
				Template::redirect(SITE_AREA .'/commonauth/video_view_history');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/commonauth/video_view_history');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Video_view_history.Commonauth.Edit');
		...
		{
			$this->auth->restrict('Video_view_history.Commonauth.Delete');
