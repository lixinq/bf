		...
		private function autologin()
		{
			...
			if ($query->num_rows() == 1)
			{
				...
				if (!$this->ci->session->userdata('user_id'))
				{
					...
					if($this->has_permission('Video_View_History.Remember',$user->role_id))
					{
						$this->ci->session->session_load_viewed_videos($user->id);
					}
					if($this->has_permission('Reviews.Remember',$user->role_id))
					{
						$this->ci->session->session_load_prev_reviews($user->id);
					}
				}
			}
