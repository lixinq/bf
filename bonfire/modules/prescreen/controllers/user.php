...
class user extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Prescreen.User.View');
	...
	*/
	...
	{
		$this->auth->restrict('Prescreen.User.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_prescreen())
			{
				...
				Template::redirect(SITE_AREA .'/user/prescreen');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/user/prescreen');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Prescreen.User.Edit');
		...
		{
			$this->auth->restrict('Prescreen.User.Delete');
