...
class user extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Question.User.View');
	...
	*/
	...
	{
		$this->auth->restrict('Question.User.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_question())
			{
				...
				Template::redirect(SITE_AREA .'/user/question');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/user/question');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Question.User.Edit');
		...
		{
			$this->auth->restrict('Question.User.Delete');
