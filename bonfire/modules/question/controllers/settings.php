...
class settings extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Question.Settings.View');
	...
	*/
	...
	{
		$this->auth->restrict('Question.Settings.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_question())
			{
				...
				Template::redirect(SITE_AREA .'/settings/question');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/settings/question');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Question.Settings.Edit');
		...
		{
			$this->auth->restrict('Question.Settings.Delete');
