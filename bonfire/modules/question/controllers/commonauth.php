...
class commonauth extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Question.Commonauth.View');
	...
	*/
	...
	{
		$this->auth->restrict('Question.Commonauth.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_question())
			{
				...
				Template::redirect(SITE_AREA .'/commonauth/question');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/commonauth/question');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Question.Commonauth.Edit');
		...
		{
			$this->auth->restrict('Question.Commonauth.Delete');
