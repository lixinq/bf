...
class reports extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Question.Reports.View');
	...
	*/
	...
	{
		$this->auth->restrict('Question.Reports.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_question())
			{
				...
				Template::redirect(SITE_AREA .'/reports/question');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/reports/question');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Question.Reports.Edit');
		...
		{
			$this->auth->restrict('Question.Reports.Delete');
