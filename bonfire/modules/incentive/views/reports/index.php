<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Incentive.Reports.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Incentive.Reports.Delete')) : ?>
				<tr>
					<td colspan="6">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Incentive.Reports.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/reports/incentive/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->incentive_company_id) ?></td>
				<?php else: ?>
