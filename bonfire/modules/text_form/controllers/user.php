...
class user extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Text_form.User.View');
	...
	*/
	...
	{
		$this->auth->restrict('Text_form.User.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_text_form())
			{
				...
				Template::redirect(SITE_AREA .'/user/text_form');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/user/text_form');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Text_form.User.Edit');
		...
		{
			$this->auth->restrict('Text_form.User.Delete');
