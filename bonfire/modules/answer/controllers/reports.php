...
class reports extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Answer.Reports.View');
	...
	*/
	...
	{
		$this->auth->restrict('Answer.Reports.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_answer())
			{
				...
				Template::redirect(SITE_AREA .'/reports/answer');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/reports/answer');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Answer.Reports.Edit');
		...
		{
			$this->auth->restrict('Answer.Reports.Delete');
