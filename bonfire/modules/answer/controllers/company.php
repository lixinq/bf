...
class company extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Answer.Company.View');
	...
	*/
	...
	{
		$this->auth->restrict('Answer.Company.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_answer())
			{
				...
				Template::redirect(SITE_AREA .'/company/answer');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/company/answer');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Answer.Company.Edit');
		...
		{
			$this->auth->restrict('Answer.Company.Delete');
