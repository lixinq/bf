...
class user extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Answer.User.View');
	...
	*/
	...
	{
		$this->auth->restrict('Answer.User.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_answer())
			{
				...
				Template::redirect(SITE_AREA .'/user/answer');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/user/answer');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Answer.User.Edit');
		...
		{
			$this->auth->restrict('Answer.User.Delete');
