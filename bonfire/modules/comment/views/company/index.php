<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Comment.Company.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Comment.Company.Delete')) : ?>
				<tr>
					<td colspan="10">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Comment.Company.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/company/comment/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->comment_user_id) ?></td>
				<?php else: ?>
