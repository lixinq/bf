...
class reports extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Comment.Reports.View');
	...
	*/
	...
	{
		$this->auth->restrict('Comment.Reports.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_comment())
			{
				...
				Template::redirect(SITE_AREA .'/reports/comment');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/reports/comment');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Comment.Reports.Edit');
		...
		{
			$this->auth->restrict('Comment.Reports.Delete');
