...
class settings extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Comment.Settings.View');
	...
	*/
	...
	{
		$this->auth->restrict('Comment.Settings.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_comment())
			{
				...
				Template::redirect(SITE_AREA .'/settings/comment');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/settings/comment');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Comment.Settings.Edit');
		...
		{
			$this->auth->restrict('Comment.Settings.Delete');
